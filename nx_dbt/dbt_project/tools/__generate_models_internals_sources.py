    ...
    "schemas": {
        "globaldomain_public": {
            ...
            "tables": {
                ...
                "products": {"description": "products table"},
                "product_history": {"description": "product history table"},
                "stores": {"description": "stores table"},
                "families": {"description": "families table"},
                "cron": {"description": "cron table"},
                "buy_execution": {"description": "buy execution table"},
                "dio_execution_selected": {"description": "dio execution selected table"},
                "engine_executions": {"description": "engine executions table"},
                "preconfigured_execution": {"description": "preconfigured execution table"},
                "reorder_execution": {"description": "reorder execution table"},
                "replenishment_executions": {"description": "replenishment executions table"},
                "store_transfers_executions": {"description": "store transfers executions table"},
                "store_transfers_execution_aggregates": {"description": "store transfers execution aggregates table"},
                "store_stock_items": {"description": "store stock items table"},
                "scenario": {"description": "scenarios table"},
                "warehouses": {"description": "warehouses table"},
                "skus": {"description": "skus table"},
                "seasons": {"description": "seasons table"},
                "sales": {"description": "sales table"},
                "warehouse_stock_items": {"description": "warehouse stock items table"},
                "databasechangelog": {"description": "liquibase database changelog table"},
            },
        }
    }
}
...
def generate_source_yaml(database_names: List[str], source_config: Dict[str, Any], output_file: str) -> None:
    """
    ...
    for db_name in database_names:
        for schema_name, schema_config in source_config["schemas"].items():
            source = {
                ...
                "schema": schema_name,
                "tables": [
                    {"name": table_name, "description": table_config["description"]}
                    for table_name, table_config in schema_config["tables"].items()
                ],
            }
            sources.append(source)

    yaml_content = {"version": 2, "sources": sources}

    with open(output_file, "w") as f:
        yaml.dump(yaml_content, f, sort_keys=False, default_flow_style=False, indent=2, Dumper=IndentDumper)


def add_schema(config: Dict[str, Any], schema_name: str, description: str, tables: Dict[str, Dict[str, str]]) -> None:
    """
    ...
    """
    config["schemas"][schema_name] = {"description": description, "tables": tables}


def add_table(config: Dict[str, Any], schema_name: str, table_name: str, description: str) -> None:
    """
    ...
    config["schemas"][schema_name]["tables"][table_name] = {"description": description}
